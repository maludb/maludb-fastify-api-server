 ...
 *
 * MaluDB concept: Reopen a closed issue-type note (requirements.md §4.5).
 * SQL objects: maludb_memory.
 * Teaches:
 *   - Clears issue_closed_at (sets NULL). Gated on memory_kind = 'issue'.
 *   - 409 conflict if the note is not an issue (type != 'issue') or is not currently closed.
 */
...
const FILE = 'notes_id_reopen-issue.ts';

export async function register(app: FastifyInstance): Promise<void> {
  app.route({
    method: ['POST'],
    url: '/v1/notes/:id/reopen-issue',
    handler: async (request, reply) => {
      ...
      if (note.issue_closed_at === null) {
        jsonError('conflict', 'Issue is not closed.', 409);
      }

      await dbExec(
        ctx,
        'UPDATE maludb_memory SET issue_closed_at = NULL, updated_at = now() WHERE memory_id = $1',
        [id],
      );
