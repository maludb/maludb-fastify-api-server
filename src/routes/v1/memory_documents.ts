...
export async function register(app: FastifyInstance): Promise<void> {
  app.route({
    ...
    handler: async (request, reply) => {
      ...
      const projects = toTextArray(body.projects);
      const subjects = toTextArray(body.subjects);
      const verbs = toTextArray(body.verbs);
      const events = toTextArray(body.events);

      const chunkObj = isObject(body.chunk) ? body.chunk : {};
      const chunkMax =
        chunkObj.max !== undefined ? Math.max(200, Number(chunkObj.max)) : 2000;
      const chunkOverlap =
